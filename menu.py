...
class slider(button):
    def __init__(self, xpos, ypos, width, height, colour, screen, function):
    ...
    def detect_mouse(self, mouse_position):
        if self.xpos+self.width >= mouse_position[0] >= self.xpos and self.ypos+self.height >= mouse_position[1] >= self.ypos:
            self.function = self.original_function + str((mouse_position[0]-self.xpos)*100/self.width)
            return True
        else:
            return False
...
def open_options_menu(): #creates a surface and then places the options menus buttons on it
    ...
    slider(210, 400, 300, 76, (255, 255, 255), screen, "set volume")]